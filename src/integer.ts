import {HpackDecodingError, HpackEncodingError} from './errors';
...
export function encodeInteger(
  ...
  if (
    ...
    throw new HpackEncodingError(
      `integer prefix must be between 1 and 8 bits, got ${prefixBits}`,
      'INTEGER_ENCODING'
    );
  }
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new HpackEncodingError(
      `integer value must be a non-negative safe integer, got ${String(
        value
      )}`,
      'INTEGER_ENCODING'
    );
  ...
  const bytes: number[] = [(firstByte & ~prefixMask) | prefixMask];
...
/** Result of decoding an integer: the value and bytes consumed. */
export interface DecodedInteger {
  value: number;
  /** Number of octets consumed, including the prefix octet. */
  length: number;
}
...
export function decodeInteger(
  ...
  if (offset >= buf.length) {
    throw new HpackDecodingError(
      'truncated header block while reading integer prefix',
      'TRUNCATED'
    );
  }
  ...
  while (true) {
    if (offset + length >= buf.length) {
      throw new HpackDecodingError(
        'truncated header block while reading integer continuation',
        'TRUNCATED'
      );
    }
