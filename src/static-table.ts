...
export const STATIC_TABLE_LENGTH = STATIC_TABLE.length - 1;
...
export function getStaticEntry(index: number): StaticEntry | undefined {
  if (index < 1 || index > STATIC_TABLE_LENGTH) {
    return undefined;
  }
 ...
 */
export function findStaticEntry(
  name: string,
  value?: string
): {index: number; exact: boolean} | undefined {
  let firstNameIndex = 0;
  for (let i = 1; i <= STATIC_TABLE_LENGTH; i++) {
